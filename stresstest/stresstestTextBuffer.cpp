...
RefHolder<FieldValue>& StresstestTextBuffer :: readValueUndefinedType(RefHolder<FieldValue>& refh, const char* word, bool implicitString) throw(Exception*) {
...
DBuffer StresstestTextBuffer :: readNumber(const char* numberString, DefSize requiedSize) throw (Exception*) {
	...
	try
   {
		...
		if (StresstestTextBuffer :: defaultFormatNumber == ND_HEX) type = VT_HEX;
		if (StresstestTextBuffer :: defaultFormatNumber == ND_DEC) type = VT_DEC;
...
void StresstestTextBuffer :: pass_space() {
